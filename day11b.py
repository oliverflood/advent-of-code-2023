...
def dist(gal_1, gal_2):
	...
	extra_space = sum([expansion-1 if (x > x1 and x < x2) or (x < x1 and x > x2) else 0 for x in x_indexes])
	extra_space += sum([expansion-1 if (y > y1 and y < y2) or (y < y1 and y > y2) else 0 for y in y_indexes])
